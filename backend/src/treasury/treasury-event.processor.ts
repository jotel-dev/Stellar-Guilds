...
export class TreasuryEventProcessor extends WorkerHost {
  ...
  async process(job: Job<TreasuryTransferJobData>): Promise<any> {
    ...
    try {
      const { eventType, guildId, amount, asset, fromAddress, toAddress, txHash, timestamp } = job.data;
      ...
      this.logger.error(`Failed to process treasury event job ${job.id}: ${errorMessage}`);
      throw error;
    }
  }
}
